...
void Robot::AutonomousPeriodic()
{
  ...
  {
    bool clawOpen = claw->OpenClaw(elapsedTime);
    if (clawOpen)
    {
      splineSection = 1;
    }
  ...
  if (splineSection == 1)
  {
    ...
    bool splineDone = swerveDrive->DriveToPose(Pose2d(0_m, -0.47_m, Rotation2d(0_deg)), elapsedTime);
    if (splineDone)
    {
      splineSection = 1.5;
  ...
  {
    bool clawClosed = claw->CloseClaw(elapsedTime);
    if (clawClosed)
    {
      splineSection = 2;
